...
function getMySettings() {
  return {
    ...
    // 💰 급여 정보
    BASE_SALARY: 2643850,              // 기본급
    WORKING_HOURS: 209,                // 월 소정근로시간
    FIXED_OVERTIME_ALLOWANCE: 412140,  // 고정연장수당
    PRODUCTION_BONUS: 528770,          // 생산장려금
    
    // ⏰ 잔업비 지급 설정
    OVERTIME_PAY_DELAY_MONTHS: 2,      // 2개월 후 지급
    ...
    // 👨‍👩‍👧‍👦 소득세 설정
    DEPENDENTS: 1,                     // 부양가족 수 (본인 1명)
    TAX_ENABLED: true,                 // 소득세 계산 여부 (반드시 true!)
...
function 메인_01_오늘잔업비계산() {
  ...
  if (!calendar) {
    Logger.log("❌ 캘린더 없음: " + settings.CALENDAR_NAME);
    return;
  }
  ...
  description += "▪️ 생산장려금: " + formatNumber(settings.PRODUCTION_BONUS) + "원\n";
  ...
  description += "  - 건강보험(" + settings.HEALTH_INSURANCE_RATE + "%): " + formatNumber(healthInsurance) + "원\n";
...
function 메인_02_이번달과거소급() {
  ...
  Logger.log("📅 과거 소급: " + formatDate(startDate) + " ~ " + formatDate(endDate));
  ...
  while (currentDate <= endDate) {
    ...
    var endOfNextDay = new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate() + 1, 6, 0, 0);
    var eventsToday = calendar.getEvents(startOfDay, endOfDay);
    var eventsNextDay = calendar.getEvents(endOfDay, endOfNextDay);
    var events = eventsToday.concat(eventsNextDay);
    ...
    description += "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n";
    description += "💰 " + currentDate.getFullYear() + "년 " + monthName + "월 " + currentDate.getDate() + "일 오늘까지 급여\n";
    description += "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n";
    description += "📅 " + workType + " | " + formatTime(startTime) + " - " + formatTime(endTime) + "\n\n";
    ...
    totalCalculated++;
    Logger.log("✅ " + formatDate(currentDate));
    
    currentDate.setDate(currentDate.getDate() + 1);
  }
  
  Logger.log("🎉 총 " + totalCalculated + "일 완료!");
}
...
function 메인_03_여러달한번에() {
  var YEAR = 2025;
  var START_MONTH = 8;   // 👈 시작 월
  var END_MONTH = 11;    // 👈 끝 월
  ...
  Logger.log("📅 " + YEAR + "년 " + START_MONTH + "월 ~ " + END_MONTH + "월 한번에 계산");
  Logger.log("=".repeat(70));
  Logger.log("");
  ...
  for (var month = START_MONTH; month <= END_MONTH; month++) {
    ...
    while (currentDate <= endDate) {
      ...
      if (alreadyCalculated) {
        Logger.log("⏭️ " + currentDate.getDate() + "일 - 이미 계산됨");
        currentDate.setDate(currentDate.getDate() + 1);
        continue;
      }
      ...
      if (!startEvent || !endEvent) {
        Logger.log("⚠️ " + currentDate.getDate() + "일 - 출퇴근 기록 없음");
        currentDate.setDate(currentDate.getDate() + 1);
        continue;
      }
      ...
      monthCalculated++;
      Logger.log("✅ " + currentDate.getDate() + "일 - " + overtimeHours + "h / " + formatNumber(Math.floor(pay)) + "원");
    ...
    Logger.log("");
    Logger.log("📊 " + month + "월 완료: " + monthCalculated + "일");
    Logger.log("");
...
function createMonthlyPayslipForMonth(year, month) {
  ...
  for (var i = 0; i < events.length; i++) {
    if (events[i].getTitle().indexOf("💵") > -1) {
      Logger.log("⏭️ " + month + "월 명세서 이미 존재");
      return;
    }
  }
  ...
  var title = "💵 " + month + "월 급여 명세서";
  var description = "";
  
  description += "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n";
  description += "💰 " + year + "년 " + month + "월 급여 명세서\n";
  description += "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n";
  ...
  if (delayedOvertime.pay > 0) {
    description += "▪️ " + delayedOvertime.month + "월 잔업비: " + formatNumber(Math.floor(delayedOvertime.pay)) + "원\n";
    description += "  (총 " + Math.floor(delayedOvertime.hours) + "시간)\n";
  }
  
  description += "\n💰 총 급여: " + formatNumber(totalGrossSalary) + "원\n\n";
  ...
  description += "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n";
  description += "📊 총 공제액: " + formatNumber(totalDeduction) + "원\n\n";
  description += "💸 실수령액: " + formatNumber(netSalary) + "원 💸\n";
  description += "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n";
  
  if (currentMonthOvertime.totalPay > 0) {
    description += "【 참고: " + month + "월 잔업비 누적 】\n";
...
function 삭제_01_전체완전삭제() {
  Logger.log("=".repeat(70));
  Logger.log("🗑️ 전체 완전 삭제");
  Logger.log("=".repeat(70));
  Logger.log("");
  ...
  Logger.log("📊 저장 데이터 " + deletedDataCount + "개 삭제");
  Logger.log("");
  
  // 2. 캘린더 이벤트 삭제 (지정된 캘린더만)
  var calendar = CalendarApp.getCalendarsByName(settings.CALENDAR_NAME)[0];
  ...
  Logger.log("📅 검색된 이벤트: " + events.length + "개");
  Logger.log("");
  
  for (var i = 0; i < events.length; i++) {
    var title = events[i].getTitle();
    
    if (title && (title.indexOf("💰") >= 0 || title.indexOf("💵") >= 0 || title.indexOf("잔업") >= 0 || title.indexOf("급여") >= 0)) {
      try {
        var eventDate = events[i].getStartTime();
        events[i].deleteEvent();
        Logger.log("✅ 삭제: " + formatDate(eventDate) + " - " + title);
        totalDeletedEvents++;
      } catch (e) {
        Logger.log("❌ 실패: " + title + " - " + e.message);
        failedEvents++;
      }
    }
  }
  ...
  Logger.log("");
  Logger.log("📊 삭제 성공: " + totalDeletedEvents + "개");
  if (failedEvents > 0) {
    Logger.log("⚠️  삭제 실패: " + failedEvents + "개");
  }
  Logger.log("");
  Logger.log("=".repeat(70));
  Logger.log("🎉 삭제 완료!");
...
function 삭제_01_전체완전삭제() {
  ...
  Logger.log("📊 데이터 " + deletedDataCount + "개 삭제");
  Logger.log("");
  ...
  Logger.log("📅 이벤트 " + totalDeletedEvents + "개 삭제");
  Logger.log("");
  Logger.log("=".repeat(70));
  Logger.log("🎉 삭제 완료!");
  Logger.log("=".repeat(70));
...
function 삭제_02_특정월완전삭제() {
  var YEAR = 2025;
  var MONTH = 11;  // 👈 여기 수정!
  
  Logger.log("=".repeat(70));
  Logger.log("🗑️ " + YEAR + "년 " + MONTH + "월 삭제");
  Logger.log("=".repeat(70));
  Logger.log("");
  ...
  Logger.log("📅 이벤트 " + totalDeleted + "개 삭제");
  Logger.log("🎉 완료!");
  Logger.log("=".repeat(70));
}
...
function 확인_01_남은이벤트찾기() {
  Logger.log("=".repeat(70));
  Logger.log("🔍 남은 이벤트 검색");
  Logger.log("=".repeat(70));
  Logger.log("");
  ...
  for (var c = 0; c < allCalendars.length; c++) {
    ...
    for (var i = 0; i < events.length; i++) {
      var title = events[i].getTitle();
      if (title && (title.indexOf("💰") >= 0 || title.indexOf("💵") >= 0 || title.indexOf("잔업") >= 0)) {
        foundEvents.push({
  ...
  if (foundEvents.length === 0) {
    Logger.log("✅ 남은 이벤트 없음!");
  } else {
    Logger.log("⚠️ 남은 이벤트: " + foundEvents.length + "개");
    Logger.log("");
...
function 확인_02_캘린더설정() {
  var settings = getUserSettings();
  
  Logger.log("=".repeat(70));
  Logger.log("📋 캘린더 설정 확인");
  Logger.log("=".repeat(70));
  Logger.log("");
  Logger.log("설정: " + settings.CALENDAR_NAME);
  Logger.log("부양가족 수: " + settings.DEPENDENTS);
  Logger.log("소득세 계산: " + settings.TAX_ENABLED);
  Logger.log("");
  ...
  if (calendar) {
    Logger.log("✅ 캘린더 찾음!");
    Logger.log("  이름: " + calendar.getName());
  } else {
    Logger.log("❌ 캘린더 없음!");
    Logger.log("");
    Logger.log("【 사용 가능한 캘린더 】");
    var allCalendars = CalendarApp.getAllCalendars();
  ...
  if (years.length === 0) {
    ...
    for (var i = 0; i < years.length; i++) {
      ...
      for (var j = 0; j < months.length; j++) {
        ...
        Logger.log("  " + month + "월: " + formatNumber(Math.floor(data.pay || 0)) + "원 (" + (data.hours || 0) + "h / " + (data.days || 0) + "일)");
      }
      Logger.log("");
    }
  }
...
function checkKoreanHoliday(date) {
  var holidayCalendar = CalendarApp.getCalendarsByName("대한민국의 공휴일")[0];
  if (holidayCalendar) {
...
/**
 * 월총급여액을 기준으로 소득세를 간이세액표에서 조회하는 함수
 * @param {number} monthlyIncome 소득세 조회용 월총급여액 (비과세 제외)
 * @param {number} dependents 부양가족 수
 * @param {boolean} taxEnabled 소득세 계산 여부
 * @return {number} 조회된 소득세액
 */
function calculateIncomeTax(monthlyIncome, dependents, taxEnabled) {
  ...
  // 소득세 간이세액표는 '천원' 단위이므로 1000으로 나눕니다.
...
/**
 * 2025년 근로소득 간이세액표 (부양가족 1명 기준)
 * @return {Object} 부양가족별 세액표
 */
...
function 임시_8월데이터수정() {
  ...
  Logger.log("=".repeat(70));
  Logger.log("📝 8월 데이터 수정");
  Logger.log("=".repeat(70));
  Logger.log("");
  
  // 현재 값 확인
  var currentPay = scriptProperties.getProperty("2025-8-total-pay");
  var currentHours = scriptProperties.getProperty("2025-8-total-hours");
  var currentDays = scriptProperties.getProperty("2025-8-total-days");
  
  Logger.log("【 수정 전 】");
  Logger.log("잔업비: " + (currentPay ? formatNumber(Math.floor(parseFloat(currentPay))) : "없음") + "원");
  Logger.log("시간: " + (currentHours || "없음") + "시간");
  Logger.log("일수: " + (currentDays || "없음") + "일");
  Logger.log("");
  
  // 올바른 값으로 수정
  var settings = getUserSettings();
  var 올바른시간 = 36;
  var 올바른잔업비 = 올바른시간 * settings.HOURLY_WAGE * settings.OVERTIME_MULTIPLIER;
  
  scriptProperties.setProperty("2025-8-total-hours", "36");
  scriptProperties.setProperty("2025-8-total-pay", 올바른잔업비.toString());
  
  Logger.log("【 수정 후 】");
  Logger.log("잔업비: " + formatNumber(Math.floor(올바른잔업비)) + "원");
  Logger.log("시간: 36시간");
  Logger.log("일수: " + currentDays + "일 (유지)");
  Logger.log("");
  Logger.log("✅ 수정 완료!");
  Logger.log("=".repeat(70));
}
...
function 임시_10월명세서재생성() {
  ...
  Logger.log("=".repeat(70));
  Logger.log("📄 10월 급여명세서 재생성");
  Logger.log("=".repeat(70));
  Logger.log("");
  
  // 기존 10월 명세서 찾아서 삭제
  var startDate = new Date(2025, 9, 1);
  var endDate = new Date(2025, 9, 31, 23, 59, 59);
  var events = calendar.getEvents(startDate, endDate);
  
  var deleted = false;
  for (var i = 0; i < events.length; i++) {
    if (events[i].getTitle().indexOf("💵") > -1 && events[i].getTitle().indexOf("10월") > -1) {
      events[i].deleteEvent();
      Logger.log("✅ 기존 명세서 삭제: " + events[i].getTitle());
      deleted = true;
    }
  }
  
  if (!deleted) {
    Logger.log("ℹ️  기존 명세서 없음");
  }
  Logger.log("");
  
  // 새로 생성
  createMonthlyPayslipForMonth(2025, 10);
  Logger.log("✅ 새 명세서 생성 완료");
  Logger.log("");
  Logger.log("=".repeat(70));
}
...
function 임시_8월상세확인() {
  ...
  var startDate = new Date(2025, 7, 1, 0, 0, 0);
  var endDate = new Date(2025, 7, 31, 23, 59, 59);
  var events = calendar.getEvents(startDate, endDate);
  
  Logger.log("=".repeat(70));
  Logger.log("📅 2025년 8월 잔업 상세 내역");
  Logger.log("=".repeat(70));
  Logger.log("");
  ...
  for (var i = 0; i < events.length; i++) {
    ...
    if (title === "💰 오늘 잔업비") {
      ...
      if (hoursMatch) {
        ...
        var pay = payMatch ? parseInt(payMatch[1].replace(/,/g, '')) : 0;
        ...
        var dayName = isWeekend ? "주말" : "평일";
        Logger.log("8월 " + eventDate.getDate() + "일 (" + dayName + "): " + hours + "시간 / " + formatNumber(pay) + "원");
      }
    }
  }
  ...
  Logger.log("📊 8월 총계");
  Logger.log("━".repeat(70));
  Logger.log("평일 잔업: " + weekdayHours + "시간");
  Logger.log("주말 잔업: " + weekendHours + "시간");
  Logger.log("총 근무일: " + count + "일");
  ...
  var scriptProperties = PropertiesService.getScriptProperties();
  var savedPay = scriptProperties.getProperty("2025-8-total-pay");
  var savedHours = scriptProperties.getProperty("2025-8-total-hours");
  var savedDays = scriptProperties.getProperty("2025-8-total-days");
  
  Logger.log("【 저장된 데이터 】");
  Logger.log("저장된 잔업비: " + (savedPay ? formatNumber(Math.floor(parseFloat(savedPay))) : "없음") + "원");
  Logger.log("저장된 시간: " + (savedHours || "없음") + "시간");
  Logger.log("저장된 일수: " + (savedDays || "없음") + "일");
  Logger.log("");
  
  if (savedHours && parseFloat(savedHours) !== totalHours) {
    Logger.log("⚠️ 불일치 발견!");
    Logger.log("  캘린더 합계: " + totalHours + "시간");
    Logger.log("  저장된 값: " + savedHours + "시간");
    Logger.log("  차이: " + (parseFloat(savedHours) - totalHours) + "시간");
  } else if (savedHours) {
    Logger.log("✅ 데이터 일치!");
  }
