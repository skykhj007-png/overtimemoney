...
function 트리거설정() {
  ...
  // 매일 오전 6시에 자동 실행 (전날 근무 계산 - 새벽 퇴근 포함)
  ScriptApp.newTrigger('메인_01_오늘잔업비계산')
    .timeBased()
    .atHour(6)
    .everyDays(1)
    .create();

  Logger.log("✅ 트리거 설정 완료! 매일 오전 6시에 자동 실행됩니다.");
}
...
function 트리거확인() {
  ...
  if (triggers.length === 0) {
    Logger.log("⚠️ 설정된 트리거가 없습니다. '트리거설정' 함수를 실행하세요.");
  } else {
    Logger.log("📋 현재 트리거 목록:");
    for (var i = 0; i < triggers.length; i++) {
      Logger.log("  - 함수: " + triggers[i].getHandlerFunction());
    }
  }
}
